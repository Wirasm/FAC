...
# Add the parent directory to the path to make the 'app' module findable
...
async def public_route():
    ...
    return {"message": "This is a public route", "status": "success"}
...
async def user_info(user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        user_data = {
            ...
            "name": user.get("name", ""),
            "claims": {
                k: v for k, v in user.items() if k not in ["sub", "email", "name"]
            },
        }

        app_logger.debug(
            f"User data extracted: id={user_id}, email={user_data['email']}"
        )

        return {"message": "User information retrieved successfully", "user": user_data}
...
async def debug_token(credentials=Depends(security)):
    ...
    try:
        ...
        app_logger.debug(
            f"Token debug - alg: {header.get('alg')}, kid: {header.get('kid')}"
        )
        app_logger.debug(
            f"Token debug - sub: {payload.get('sub')}, exp: {payload.get('exp')}"
        )
