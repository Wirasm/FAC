...
def setup_cors(app: FastAPI) -> None:
    ...
    if settings.CORS_ALLOW_CREDENTIALS:
        ...
        if settings.DEBUG and origins == ["*"]:
            ...
            app_logger.warning("CORS is configured to allow specific origins in development mode with credentials.")
    else:
        # If not using credentials, we can use wildcard
        if settings.DEBUG or origins == ["*"]:
            origins = ["*"]
            app_logger.warning("CORS is configured to allow all origins. This should not be used in production.")
